...
ZTEST(ring_buffer, test_init_buffer_is_empty) {
    ...
    zassert_equal(ring_buffer_init(&rb, storage, TEST_BUFFER_CAPACITY), RING_BUFFER_SUCCESS,
                  "Initialization failed");

    zassert_true(ring_buffer_is_empty(&rb), "Buffer should be empty after initialization");
    zassert_false(ring_buffer_is_full(&rb), "Buffer should not be full after initialization");
    zassert_equal(ring_buffer_get_size(&rb), 0U, "Buffer size should be zero after initialization");
}
...
ZTEST(ring_buffer, test_put_one_element) {
    ...
    zassert_equal(ring_buffer_put(&rb, 10), RING_BUFFER_SUCCESS, "Put operation failed");

    zassert_false(ring_buffer_is_empty(&rb), "Buffer should not be empty after one put");
...
ZTEST(ring_buffer, test_get_one_element) {
    ...
    zassert_equal(ring_buffer_put(&rb, 42), RING_BUFFER_SUCCESS, "Put operation failed");
    zassert_equal(ring_buffer_get(&rb, &value), RING_BUFFER_SUCCESS, "Get operation failed");

    zassert_equal(value, 42, "Returned value does not match inserted value");
...
ZTEST(ring_buffer, test_drain_fails_with_invalid_capacity) {
    ...
    zassert_equal(drained_count, 0U, "Drained count should be zero on invalid capacity");
...
ZTEST(ring_buffer, test_put_fails_when_full) {
    ...
    zassert_true(ring_buffer_is_full(&rb), "Buffer should be full after reaching capacity");
    zassert_equal(ring_buffer_get_size(&rb), TEST_BUFFER_CAPACITY,
                  "Buffer size should equal capacity");

    zassert_equal(ring_buffer_put(&rb, 5), RING_BUFFER_ERROR_FULL,
...
ZTEST(ring_buffer, test_get_fails_when_empty) {
    ...
    zassert_equal(ring_buffer_get(&rb, &value), RING_BUFFER_ERROR_EMPTY,
                  "Get should fail when buffer is empty");

    zassert_equal(ring_buffer_get_size(&rb), 0U, "Buffer size should remain zero");
}
...
ZTEST(ring_buffer, test_wrap_around_preserves_fifo_order) {
    ...
    zassert_true(ring_buffer_is_full(&rb), "Buffer should be full after wraparound refill");
...
ZTEST(ring_buffer, test_init_fails_with_null_storage) {
    ...
    zassert_equal(ring_buffer_init(&rb, NULL, TEST_BUFFER_CAPACITY), RING_BUFFER_ERROR_INIT_FAILED,
...
ZTEST(ring_buffer, test_init_fails_with_zero_capacity) {
    ...
    zassert_equal(ring_buffer_init(&rb, storage, 0U), RING_BUFFER_ERROR_INIT_FAILED,
...
ZTEST(ring_buffer, test_put_fails_with_null_buffer) {
    zassert_equal(ring_buffer_put(NULL, 123), RING_BUFFER_ERROR_INVALID_ARGUMENT,
...
ZTEST(ring_buffer, test_get_fails_with_null_buffer) {
    ...
    zassert_equal(ring_buffer_get(NULL, &value), RING_BUFFER_ERROR_INVALID_ARGUMENT,
...
ZTEST(ring_buffer, test_get_fails_with_null_output_pointer) {
    ...
    zassert_equal(ring_buffer_get(&rb, NULL), RING_BUFFER_ERROR_INVALID_ARGUMENT,
...
ZTEST(ring_buffer, test_clear_resets_buffer_state) {
    ...
    zassert_true(ring_buffer_is_empty(&rb), "Buffer should be empty after clear");
    zassert_false(ring_buffer_is_full(&rb), "Buffer should not be full after clear");
    zassert_equal(ring_buffer_get_size(&rb), 0U, "Buffer size should be zero after clear");
}
...
ZTEST(ring_buffer, test_clear_allows_buffer_reuse) {
    ...
    zassert_equal(ring_buffer_get(&rb, &value), RING_BUFFER_SUCCESS, NULL);
    zassert_equal(value, 33, "Buffer should be reusable after clear");
}
...
ZTEST(ring_buffer, test_capacity_one_behavior) {
    ...
    zassert_equal(ring_buffer_init(&rb, storage, 1U), RING_BUFFER_SUCCESS,
    ...
    zassert_equal(ring_buffer_put(&rb, 100), RING_BUFFER_ERROR_FULL,
...
ZTEST(ring_buffer, test_query_helpers_with_null_buffer) {
    ...
    zassert_equal(ring_buffer_get_size(NULL), 0U, "NULL buffer size should be reported as zero");
}
